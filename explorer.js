...
// Path-style URLs, ex: https://s3.amazonaws.com/mybucket1/index.html
// eslint-disable-next-line no-unused-vars
function object2urlkhulnasoft(bucket, key, escape = false) {
  const enckey = key.split('/').map(x => encodeURIComponent(x)).join('/');
  const rc = `${document.location.protocol}//${bucket}/${enckey}`;
  return escape ? htmlEscape(rc) : rc;
}
...
function ViewController($scope, SharedService) {
    ...
    $scope.renderObject = (data, _type, full) => {
        ...
        const hrefv = object2urlkhulnasoft($scope.view.settings.bucket, data);
...
function SettingsController($scope, SharedService) {
    ...
    $scope.settings = {
        auth: 'anon', region: 'us-west-1', bucket: 'packages-dev.khulnasoft.com', entered_bucket: 'packages-dev.khulnasoft.com', selected_bucket: '', view: 'folder', delimiter: '/', prefix: '',
    };
...
$(document).ready(() => {
    ...
    settings_btn =  document.getElementById("submitbtn")
    settings_btn.click()

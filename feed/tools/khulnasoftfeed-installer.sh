...
FEED_PATH="khulnasoft-feed.tar.gz"
LOCAL="no"
HASH_PATH="khulnasoft-feed_hash"
CDB="yes"
HASH="no"
TOKEN=""
...
help() {
    ...
    echo "    -t TOKEN       [Required] Set the Token for download the Khulnasoft Feed."
    echo
    exit $1
}
...
/var/ossec/bin/khulnasoft-analysisd -t
STATUS=$?
if [[ $STATUS -ne 0 ]] ; then
...
/var/ossec/bin/khulnasoft-analysisd -t -c backup/test/ossec.conf >> test.txt 2>&1
...
# Step 3: stop the Khulnasoft manager

systemctl stop khulnasoft-manager
...
systemctl start khulnasoft-manager
systemctl status khulnasoft-manager
